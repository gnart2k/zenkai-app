...
import { JobSeekerSidebar } from './_components/job-seeker-sidebar';
...
export default function Layout({ children }: { children: React.ReactNode }) {
  ...
  const generateBreadcrumbs = (path: string) => {
    ...
    if (hrIndex === -1) {
      ...
      return [{ title: 'Home', href: '/job-seeker' }];
    }
    
    // Start breadcrumbs from 'hr' onwards
    const hrSegments = segments.slice(hrIndex);
    const breadcrumbs = [{ title: 'Home', href: '/job-seeker' }];

    let currentPath = '/job-seeker';
  ...
  return (
    <section className="flex flex-col min-h-screen">
      <div>
        ...
        >
          <JobSeekerSidebar variant="inset" />
          <SidebarInset>
